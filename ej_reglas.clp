...
(defrule calculaPerimetros
    ?h<-(circunferencia (nombre ?n) (radio ?r&~nil) (perimetro nil))
	=>
	(modify ?h (perimetro (* 6.28 ?r)))
)

(defrule calculaOquedadCircunferenciasConcentricas
	(circunferencia (nombre ?n1) (radio ?x&~nil))
    (circunferencia (nombre ?n2) (radio ?y&~nil&:(> ?y ?x)))
	=>
	(assert (oquedad (- ?y ?x)))
)

(defrule calculaRadio
    ?h<-(circunferencia (nombre ?n) (radio nil) (perimetro ?p&~nil))
    =>
