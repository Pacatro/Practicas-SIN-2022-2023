...
//		k3smoothing_p2 = 3. * ksmoothing_p2;
		...
		for (auto pair : memory) {
			...
			if (pair.second) {
				...
				int total = pair.first->numWins + pair.first->numLost + pair.first->numTies;
				...
				if(iWon){
					...
					if(total > MAXOCCURRENCES){
						if(random == 2 && pair.first->numLost > MAXOCCURRENCES)
							pair.first->numLost--;
						else if(pair.first->numTies > MAXOCCURRENCES)
							pair.first->numTies--;
					}
					...
					if(pair.first->numLost < MAXOCCURRENCES)
						pair.first->numLost++;
					if(total > MAXOCCURRENCES){
						if(random == 2 && pair.first->numWins > MAXOCCURRENCES)
							pair.first->numWins--;
						else if(pair.first->numTies > MAXOCCURRENCES)
							pair.first->numTies--; 
					}
					...
					if(total > MAXOCCURRENCES){
						if(random == 2 && pair.first->numWins > MAXOCCURRENCES)
							pair.first->numWins--;
						else if(pair.first->numLost > MAXOCCURRENCES)
							pair.first->numLost--;
					} 
				}
			}
